package fr.elowyr.core.utils.commands;

import fr.elowyr.core.utils.Utils;
import fr.elowyr.core.utils.commands.annotations.Command;
import fr.elowyr.core.utils.commands.annotations.Completer;
...
public class CommandFramework implements CommandExecutor {
		...
		for (int i = args.length; i >= 0; i--) {
			...
			if (commandMap.containsKey(cmdLabel)) {
				...
				if ((!command.isConsole()) && (!(sender instanceof Player))) {
					sender.sendMessage("Désolé, Cette commande est uniquement disponible en jeu!");
					return true;
				}
				...
				if (!hasPerm) {
					sender.sendMessage(ChatColor.RED + "Désolé, Vous n'avez pas la permission d'executer cette commande!");
					return true;
				}
	...
	private void defaultCommand(CommandArgs args) {
		args.getSender().sendMessage(Utils.color("&cDésolé, la commande " + args.getLabel() + " n'existe pas !"));
	}
}
